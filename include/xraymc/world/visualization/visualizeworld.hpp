...
#include "xraymc/world/visualization/vizualizationprops.hpp"
            ...
            if (res.valid() && res.intersection < line_intersection) {
                if (m_colorByValue.contains(res.item)) {
                    if (m_colorByValueLogScale) {
                        ...
                        const auto color = m_setlowest_dose_color_to_white && cind == 0 ? std::array<U, 3> { 1, 1, 1 } : turboColorMap<U>()[cind];
