...
protected:
    ...
    void voxelTransportTest1(ParticleType auto& p, RandomState& state)
    {
    ...
    template <std::uint_fast8_t IGNOREIDX = 255>
    void voxelTransportTest2(ParticleType auto& p, RandomState& state)
    {
        ...
        do {
            ...
            const auto rand = state.randomUniform();
            if (rand < prob) {
                // no interaction, we translate to next plane and update direction and energy if needed
                p.translate(tstep[nextPlane]);
                xyz[nextPlane] = xyz[nextPlane] + (p.dir[nextPlane] < 0 ? -1 : 1);
                if (xyz[nextPlane] >= m_dim[nextPlane]) {
                    return;
                ...
                for (std::size_t i = 0; i < 3; ++i) {
                    tstep[i] -= tstep[nextPlane];
                }
                const auto plane = p.dir[nextPlane] < 0 ? m_aabb[nextPlane] + xyz[nextPlane] * m_spacing[nextPlane] : m_aabb[nextPlane] + (xyz[nextPlane] + 1) * m_spacing[nextPlane];
                tstep[nextPlane] = (plane - p.pos[nextPlane]) / p.dir[nextPlane];
            } else {
                // interaction
                const auto step_correction = -std::log(rand) / (att.sum() * density);
                p.translate(step_correction);
