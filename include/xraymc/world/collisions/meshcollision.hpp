...
namespace collision {
    ...
    bool collideBasicShape(const xraymc::Triangle& t1, const xraymc::Triangle& t2)
    {
    ...
    bool collideBasicShape(const xraymc::Triangle& tri, const xraymc::Tetrahedron& tet)
    {
        const auto& vertices = tet.vertices();
        const auto t1 = xraymc::Triangle(vertices[0], vertices[1], vertices[2]);
        const auto t2 = xraymc::Triangle(vertices[1], vertices[0], vertices[3]);
        const auto t3 = xraymc::Triangle(vertices[2], vertices[3], vertices[0]);
        const auto t4 = xraymc::Triangle(vertices[3], vertices[2], vertices[1]);
    ...
    bool collideBasicShape(const xraymc::Tetrahedron& tet, const xraymc::Triangle& tri)
    {
        return collideBasicShape(tri, tet);
    }

    bool collideBasicShape(const xraymc::Tetrahedron& tet1, const xraymc::Tetrahedron& tet2)
    {
    ...
    bool collideBasicShape(const std::vector<xraymc::Triangle>& tri1, const std::vector<xraymc::Triangle>& tri2)
    {
    ...
    bool collideBasicShape(const std::vector<xraymc::Triangle>& tri1, const std::vector<xraymc::Tetrahedron>& tet2)
    {
    ...
    bool collideBasicShape(const std::vector<xraymc::Tetrahedron>& tet2, const std::vector<xraymc::Triangle>& tri1)
    {
        return collideBasicShape(tri1, tet2);
    }

    bool collideBasicShape(const std::vector<xraymc::Tetrahedron>& tet1, const std::vector<xraymc::Tetrahedron>& tet2)
    {
    ...
    concept TriangleMeshType = requires(T a) {
        { a.triangles() } -> std::convertible_to<std::vector<xraymc::Triangle>>;
    };

    template <typename T>
    concept TetrahedronMeshType = requires(T a) {
        { a.tetrahedrons() } -> std::convertible_to<std::vector<xraymc::Tetrahedron>>;
    };
