...
/**
 * @file betheHeitlerCrossSection.hpp
 * @brief Semi-relativistic Bethe-Heitler bremsstrahlung spectrum model for a tungsten anode.
 *
 * Implements the Tucker, Barnes & Chakraborty (1991) adaptation of the Birch & Marshall (1979)
 * analytical model, which is the standard physics basis for diagnostic X-ray tube spectrum
 * simulation.
 *
 * ## Physical model summary
 *
 * An electron of initial kinetic energy T₀ (the tube voltage) loses energy as it penetrates the
 * tungsten target. At each depth x [mg/cm²] and residual energy fraction u = T/T₀, it emits
 * bremsstrahlung photons according to the semi-relativistic Bethe-Heitler cross section with a
 * Coulomb correction. Photons produced at depth x must then survive self-filtration through the
 * remaining tungsten before reaching the detector.
 *
 * The electron transport is modelled by two 2-D tables (`CP100_F`, `CP100_M`) derived from Monte
 * Carlo simulations at 100 keV, which are scaled to other tube voltages via the Thomson–Widdington
 * range law.
 *
 * ## Call graph for spectrum generation
 *
 * ```
 * betheHeitlerSpectra(T0, hv, takeoffAngle)
 * ├── electronDensity(u, x, T0)
 * │   ├── numberFractionF(x, T0)  →  ThomsonWiddingtonLaw()
 * │   ├── electronDensity_F(u, x)  →  CP100_F(), bilinearInterpolate()
 * │   ├── numberFractionM(x, T0)  →  ThomsonWiddingtonRange()
 * │   └── electronDensity_M(u, x)  →  CP100_M(), bilinearInterpolate()
 * ├── betheHeitlerCrossSection(hv, Ti)  →  PHI_BAR()
 * └── tungstenFiltration(att, x, sinθ)
 *
 * characteristicTungstenKedge(T0, takeoffAngle)
 * └── betheHeitlerSpectra()  (called per K-line energy)
 * ```
 *
 * ## References
 * - R. Birch & M. Marshall, "Computation of bremsstrahlung X-ray spectra and comparison with
 *   spectra measured with a Ge(Li) detector", *Phys. Med. Biol.* 24(3), 505–517 (1979).
 * - D.M. Tucker, G.T. Barnes & D.P. Chakraborty, "Semiempirical model for generating tungsten
 *   target X-ray spectra", *Med. Phys.* 18(2), 211–218 (1991).
 */
#pragma once
...
/**
 * @namespace xraymc::BetheHeitlerCrossSection
 * @brief Semi-relativistic Bethe-Heitler bremsstrahlung spectrum model for a tungsten anode.
 *
 * Implements the Tucker, Barnes & Chakraborty (1991) / Birch & Marshall (1979) analytical model
 * for computing the X-ray photon fluence spectrum emitted by a tungsten anode X-ray tube. All
 * functions are free `static` functions; there is no class to instantiate.
 *
 * ### Primary entry points
 * | Function                         | Purpose                                                    |
 * |----------------------------------|------------------------------------------------------------|
 * | `betheHeitlerSpectra()`          | Continuous bremsstrahlung fluence at a single photon energy |
 * | `characteristicTungstenKedge()`  | Fluences of the four dominant tungsten Kα/Kβ lines         |
 *
 * ### Section 1 — Electron diffusion in tungsten
 * Electrons slow down and scatter as they penetrate the anode. Their distribution is
 * parameterised by two 2-D look-up tables generated by Monte Carlo at 100 keV:
 * - `CP100_F`: forward-scattered electron density (depth grid `CP100_F_x()`).
 * - `CP100_M`: multiply-scattered electron density (depth grid `CP100_M_x()`).
 *
 * Both tables share the normalised energy axis `CP100_u()` (u = T/T₀). At other tube
 * voltages the depth axis is rescaled via `ThomsonWiddingtonRange()`, and the forward/
 * multiply fractions are given by `numberFractionF()` / `numberFractionM()`.
 * `electronDensity()` combines both components into a single ρ(u, x, T₀).
 *
 * ### Section 2 — Bethe-Heitler cross section and spectrum integration
 * `betheHeitlerCrossSection()` computes the differential photon emission probability
 * dσ/dhν for a single electron energy, incorporating a relativistic Coulomb correction.
 * `tungstenFiltration()` applies Beer-Lambert self-attenuation for photons escaping
 * through the target at the anode take-off angle.
 * `betheHeitlerSpectra()` integrates these over the full electron distribution.
 */
namespace BetheHeitlerCrossSection {
    ...
    /** @brief Atomic number of tungsten (Z = 74), used as the anode target in the Bethe-Heitler model. */
