...
template <typename T = int, typename U>
std::vector<T> generateBox(const std::array<std::size_t, 3>& dim, const std::array<U, 3>& spacing = { 1, 1, 1 })
{
    const auto s = std::reduce(dim.cbegin(), dim.cend(), std::size_t { 1 }, std::multiplies<>());
...
bool dose_test()
{
    ...
    std::cout << " Total detector dose: " << std::accumulate(dose_buffer.cbegin(), dose_buffer.cend(), 0.0) << " mGy" << std::endl;
